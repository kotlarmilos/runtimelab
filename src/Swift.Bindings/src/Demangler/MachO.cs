...
namespace Xamarin
{
            ...
            for (int i = 0; i < ncmds; i++)
            {
                ...
                switch (cmd)
                {
                    ...
                    case MachO.LoadCommands.SymTab:
                        var dySymTab = new SymTabLoadCommand();
                        dySymTab.cmd = reader.ReadUInt32();
                        dySymTab.cmdsize = reader.ReadUInt32();
                        dySymTab.symoff = reader.ReadUInt32();
                        dySymTab.nsyms = reader.ReadUInt32();
                        dySymTab.stroff = reader.ReadUInt32();
                        dySymTab.strsize = reader.ReadUInt32();
                        dySymTab.nlist = SymTabLoadCommand.ReadNList(!is64bitheader, reader, startOffset, dySymTab.symoff,
                            dySymTab.nsyms, dySymTab.stroff);
                        lc = dySymTab;
                        break;
                    default:
                        lc = new LoadCommand();
    ...
    public class SymTabLoadCommand : LoadCommand
    {
        ...
        public NListEntry[] nlist = new NListEntry[0];

        public static NListEntry[] ReadNList(bool is32Bit, BinaryReader reader, long startOffset, uint symoff, uint nsyms, uint stroff)
        {
            var nlist = new NListEntry[nsyms];
            var savePos = reader.BaseStream.Position;
            try
            {
                reader.BaseStream.Seek(startOffset + symoff, SeekOrigin.Begin);
                for (uint i = 0; i < nsyms; i++)
                    nlist[i] = NListEntry.FromBinaryReader(reader, is32Bit);
        ...
        public IEnumerable<NListEntry> PublicSymbols =>
            nlist.Where((nle, i) => nle.IsPublic && !nle.IsSymbolicDebuggerEntry);
    }
}
